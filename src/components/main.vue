<template>
	<v-stepper v-model="e1" class="mb-n10 pb-16">
		<v-stepper-header>
			<v-stepper-step color="green" :complete="e1 > 1" step="1"> Get Started </v-stepper-step>

			<v-divider></v-divider>

			<v-stepper-step color="green" :complete="e1 > 2" step="2"> Add Providers </v-stepper-step>

			<v-divider></v-divider>

			<v-stepper-step color="green" :complete="e1 > 3" step="3"> Add Services </v-stepper-step>
		...
		<v-stepper-items>
			<v-stepper-content step="1">
				<v-card elevation="0">
					<v-container>
						...
						>
						<div id="step1-box" style="margin-left: 20%; margin-right: 20%" class="mt-15">
							<v-row class="mt-n5">
								<v-col cols="12" sm="12">
									<span class="black--text dark-v">Organization/Provider Name</span>
									<v-text-field
										label="Org. Name"
										outlined
										clearable
							...
							<v-row class="mt-n5">
								<v-col cols="12" sm="12">
									<span class="black--text">Organization/Provider Location</span>
									<v-text-field
										label="Org. Location"
										outlined
										clearable
						...
						<v-btn @click="goToEle2(true)" class="mx-2 mt-3 fr" fab dark small color="indigo">
			...
			<v-stepper-content step="2">
				...
				<v-btn @click="goToEle3()" class="mx-2 mt-10 fr" fab dark small color="indigo">
					<v-icon dark> mdi-arrow-right </v-icon>
				</v-btn>
			...
			<v-stepper-content step="3">
				...
				<v-btn class="mx-2 fl mt-10" @click="goToEle2(false)" fab dark small color="indigo">
					<v-icon dark> mdi-arrow-left </v-icon>
				</v-btn>
				<span class="mt-12 ml-2 mr-2 fl">Back</span>

				<v-btn @click="goToEle4()" class="mx-2 fr mt-11" id="next-service-btn" fab dark small color="indigo">
					<v-icon dark> mdi-arrow-right </v-icon>
				</v-btn>
			...
			<v-stepper-content step="4">
				...
				<v-btn class="mx-2 fl mt-10" @click="goToEle3()" fab dark small color="indigo">
					<v-icon dark> mdi-arrow-left </v-icon>
				</v-btn>
				<span class="mt-12 ml-2 mr-2 fl">Back</span>

				<v-btn id="btn-submit" @click="submit()" class="mx-2 fr mt-9" fab dark small color="green">
					<v-icon dark> mdi-arrow-right </v-icon>
				</v-btn>
...
export default {
	...
	mounted() {
		...
		if (this.$vuetify.theme.dark) {
			document.getElementsByClassName('black--text').forEach((element) => {
				let c_list = element.classList.toString().replace('black--text', ' ');
				element.classList = c_list;
			});
